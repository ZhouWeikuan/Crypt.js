...
local tlshelper     = require "http.tlshelper"

---! interface for ws:// or wss://
local SSLCTX_SERVER = nil
local function gen_interface(isSec, fd)
    if isSec then
        if not SSLCTX_SERVER then
            -- gen cert and key, something like
            -- openssl req -x509 -newkey rsa:2048 -days 3650 -nodes -keyout server-key.pem -out server-cert.pem
            -- but it is not truely valid, you should get it from CA authority
            SSLCTX_SERVER   = tlshelper.newctx()
            local certfile  = skynet.getenv("certfile") or error("no certfile like config/server-cert.pem found")
            local keyfile   = skynet.getenv("keyfile") or error("no keyfile like config/server-key.pem found")
            SSLCTX_SERVER:set_cert(certfile, keyfile)
        end

        local tls_ctx = tlshelper.newtls("server", SSLCTX_SERVER)
        return {
            init    = tlshelper.init_responsefunc(fd, tls_ctx),
            close   = tlshelper.closefunc(tls_ctx),
            read    = tlshelper.readfunc(fd, tls_ctx),
            write   = tlshelper.writefunc(fd, tls_ctx),
        }
    else
        return {
            init    = nil,
...
ws.new = function (id, isSec, handler, conf)
    local interface = gen_interface(isSec, id)
    ...
    if code then
        response(id, interface.write, code, result)
        socket.close(id)
...
function ws:recv_frame()
    local data, err
    xpcall(function ()
        data, err = self.interface.read(2)
    end,
    function(msg)
        data = nil
        err = msg and msg or ""
    ...
    elseif payloadlen == 126 then
        local h_data, err = self.interface.read(2)
        if not h_data then
        ...
        local l_data, err = self.interface.read(8)
        if not l_data then
    ...
    if mask_frame then
        local mask, err = self.interface.read(4)
        if not mask then
    ...
    if frame_length > 0 then
        local fdata, err = self.interface.read(frame_length)
        if not fdata then
